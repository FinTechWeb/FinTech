import { useState, useRef } from "react";
import "./AccountLimits.css";

const AVATAR_SRC = "data:image/png;base64,";

const NAV_ITEMS = [
  { id: "home",          label: "Home",                Icon: HomeIcon },
  { id: "history",       label: "Transaction History", Icon: HistoryIcon },
  { id: "beneficiaries", label: "Beneficiaries",       Icon: BeneficiariesIcon },
  { id: "limits",        label: "Account limits",      Icon: ShieldIcon },
  { id: "profile",       label: "Profile",             Icon: PersonIcon },
];

export default function AccountLimits({ onNavigate }) {
  const [step, setStep]                     = useState("limits");
  const [residenceFile, setResidenceFile]   = useState(null);
  const [fundsFile, setFundsFile]           = useState(null);
  const [dragResidence, setDragResidence]   = useState(false);
  const [dragFunds, setDragFunds]           = useState(false);
  const [reason, setReason]                 = useState("");
  const [limitVal, setLimitVal]             = useState("");
  const residenceRef = useRef(null);
  const fundsRef     = useRef(null);

  const onDrop = (e, setter, setDrag) => {
    e.preventDefault(); setDrag(false);
    const f = e.dataTransfer.files[0]; if (f) setter(f);
  };

  const fullscreen = ["upload-residence","upload-funds","review","success"].includes(step);

  return (
    <div className="al-root">

      {/* ─── Sidebar ─── */}
      <aside className="al-sidebar">
        <div className="al-logo">Logo</div>
        <nav className="al-nav">
          {NAV_ITEMS.map(({ id, label, Icon }) => (
            <button
              key={id}
              className={`al-nav-btn${id === "limits" ? " al-nav-btn--active" : ""}`}
              onClick={() => onNavigate?.(id)}
            >
              <Icon size={18} color={id === "limits" ? "#fff" : "#666"} />
              <span className="al-nav-label">{label}</span>
            </button>
          ))}
        </nav>
      </aside>

      {/* ─── Main ─── */}
      <div className="al-main">

        {/* Topbar — hidden on fullscreen steps */}
        {!fullscreen && (
          <header className="al-topbar">
            <div className="al-topbar-spacer" />
            <div className="al-user-area">
              <div className="al-avatar-ring">
                <img src={AVATAR_SRC} alt="Kabir Akinola" className="al-avatar-img" />
              </div>
              <span className="al-user-name">Kabir Akinola</span>
              <ChevronIcon />
              <div className="al-bell">
                <BellIcon size={20} color="#222" />
                <span className="al-bell-dot" />
              </div>
            </div>
          </header>
        )}

        {/* ═══════════════ STEP 1 — Limits ═══════════════ */}
        {step === "limits" && (
          <div className="al-page">
            {/* Mobile header */}
            <header className="al-mob-header">
              <button className="al-icon-btn" onClick={() => onNavigate?.("home")}>
                <ArrowLeft size={22} color="#111" />
              </button>
              <span className="al-mob-title">Account limits</span>
              <span className="al-mob-spacer" />
            </header>

            <div className="al-page-body">
              <h1 className="al-page-title">Account limits</h1>
              <p className="al-section-label">Current limits</p>
              <div className="al-card">
                <div className="al-limits-row">
                  <div className="al-limit-col">
                    <span className="al-limit-label">Daily limit</span>
                    <span className="al-limit-val">20,000 UK</span>
                  </div>
                  <div className="al-limit-col">
                    <span className="al-limit-label">Yearly limit</span>
                    <span className="al-limit-val">1,000,000 UK</span>
                  </div>
                </div>
                <button className="al-btn-red" onClick={() => setStep("modal")}>
                  Increase Limits
                </button>
              </div>
            </div>
          </div>
        )}

        {/* ═══════════════ STEP 2 — Modal ═══════════════ */}
        {step === "modal" && (
          <div className="al-page">
            <div className="al-page-body">
              <h1 className="al-page-title">Account limits</h1>
              <p className="al-section-label">Current limits</p>
              <div className="al-card al-card--dim">
                <div className="al-limits-row">
                  <div className="al-limit-col">
                    <span className="al-limit-label">Daily limit</span>
                    <span className="al-limit-val">20,000 UK</span>
                  </div>
                  <div className="al-limit-col">
                    <span className="al-limit-label">Yearly limit</span>
                    <span className="al-limit-val">1,000,000 UK</span>
                  </div>
                </div>
                <button className="al-btn-red al-btn--dim">Increase Limits</button>
              </div>
            </div>

            <div className="al-backdrop" onClick={() => setStep("limits")}>
              <div className="al-modal" onClick={e => e.stopPropagation()}>
                <div className="al-field-group">
                  <label className="al-field-label-text">Reason for request</label>
                  <div className="al-sel-wrap">
                    <select className="al-select" value={reason} onChange={e => setReason(e.target.value)}>
                      <option value="">— Select —</option>
                      <option>Business needs</option>
                      <option>Personal reasons</option>
                      <option>Investment</option>
                      <option>Other</option>
                    </select>
                    <span className="al-sel-arrow"><ChevronIcon size={15} /></span>
                  </div>
                </div>
                <div className="al-field-group">
                  <label className="al-field-label-text">Enter limit</label>
                  <input
                    type="number"
                    className="al-input"
                    placeholder="0"
                    value={limitVal}
                    onChange={e => setLimitVal(e.target.value)}
                  />
                </div>
                <button className="al-btn-red al-btn-full" onClick={() => setStep("upload-residence")}>
                  Proceed
                </button>
              </div>
            </div>
          </div>
        )}

        {/* ═══════════════ STEP 3 — Upload residence ═══════════════ */}
        {step === "upload-residence" && (
          <UploadPage
            title="Upload proof of residence"
            subtitle="You are required to upload a proof of residence to proceed"
            file={residenceFile}
            setFile={setResidenceFile}
            drag={dragResidence}
            setDrag={setDragResidence}
            inputRef={residenceRef}
            onDrop={onDrop}
            onBack={() => setStep("modal")}
            onNext={() => setStep("upload-funds")}
          />
        )}

        {/* ═══════════════ STEP 4 — Upload funds ═══════════════ */}
        {step === "upload-funds" && (
          <UploadPage
            title="Upload proof of funds"
            subtitle={`You are required to provide a proof of the source of your funds\nfor security purposes. Upload an appointment letter or bank statement`}
            file={fundsFile}
            setFile={setFundsFile}
            drag={dragFunds}
            setDrag={setDragFunds}
            inputRef={fundsRef}
            onDrop={onDrop}
            onBack={() => setStep("upload-residence")}
            onNext={() => setStep("review")}
          />
        )}

        {/* ═══════════════ STEP 5 — Review ═══════════════ */}
        {step === "review" && (
          <div className="al-status-page">
            <button className="al-go-home" onClick={() => { setStep("limits"); onNavigate?.("home"); }}>
              <ArrowLeft size={15} color="#E53E3E" />
              <span>Go home</span>
            </button>
            <div className="al-status-body">
              <div className="al-status-circle al-status-circle--amber">
                <ClockIcon size={40} color="#fff" />
              </div>
              <h1 className="al-status-title">Limit increase under review</h1>
              <p className="al-status-text">
                Your request to upgrade account limits is currently{"\n"}
                being processed. Please check back later
              </p>
              <button className="al-btn-amber" onClick={() => setStep("success")}>
                Request Approved →
              </button>
            </div>
          </div>
        )}

        {/* ═══════════════ STEP 6 — Success ═══════════════ */}
        {step === "success" && (
          <div className="al-status-page">
            <div className="al-status-body">
              <div className="al-star-badge">
                <CheckIcon size={36} color="#fff" />
              </div>
              <h1 className="al-status-title">Limit increased successfully</h1>
              <p className="al-status-text">Your account limit has been increased!</p>
              <button
                className="al-btn-red al-btn-done"
                onClick={() => { setStep("limits"); onNavigate?.("home"); }}
              >
                Done
              </button>
            </div>
          </div>
        )}

      </div>
    </div>
  );
}

/* ─── Upload Page (reusable) ─── */
function UploadPage({ title, subtitle, file, setFile, drag, setDrag, inputRef, onDrop, onBack, onNext }) {
  return (
    <div className="al-upload-page">
      <button className="al-icon-btn al-back-abs" onClick={onBack}>
        <ArrowLeft size={22} color="#111" />
      </button>
      <h1 className="al-upload-title">{title}</h1>
      <p className="al-upload-sub">{subtitle}</p>
      <div
        className={`al-dropzone${drag ? " al-dropzone--over" : ""}`}
        onDragOver={e => { e.preventDefault(); setDrag(true); }}
        onDragLeave={() => setDrag(false)}
        onDrop={e => onDrop(e, setFile, setDrag)}
        onClick={() => inputRef.current.click()}
      >
        <input
          ref={inputRef}
          type="file"
          style={{ display: "none" }}
          onChange={e => setFile(e.target.files[0])}
        />
        <UploadIcon size={36} color="#E53E3E" />
        {file ? (
          <p className="al-filename">{file.name}</p>
        ) : (
          <>
            <p className="al-drop-text"><span className="al-drop-link">Click to Upload</span> or drag and drop</p>
            <p className="al-drop-hint">(Max. File size: 25 MB)</p>
          </>
        )}
      </div>
      <button
        className={`al-btn-next${file ? " al-btn-next--active" : ""}`}
        onClick={() => file && onNext()}
      >
        Next
      </button>
    </div>
  );
}

/* ─── Icons ─── */
function HomeIcon({ size = 20, color = "#666" }) {
  return <svg width={size} height={size} viewBox="0 0 24 24" fill="none">
    <path d="M3 9.5L12 3l9 6.5V20a1 1 0 01-1 1H4a1 1 0 01-1-1V9.5z" stroke={color} strokeWidth="1.7" strokeLinejoin="round"/>
    <path d="M9 21V12h6v9" stroke={color} strokeWidth="1.7" strokeLinecap="round"/>
  </svg>;
}
function HistoryIcon({ size = 20, color = "#666" }) {
  return <svg width={size} height={size} viewBox="0 0 24 24" fill="none">
    <circle cx="12" cy="12" r="9" stroke={color} strokeWidth="1.7"/>
    <path d="M12 7v5l3 3" stroke={color} strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round"/>
  </svg>;
}
function BeneficiariesIcon({ size = 20, color = "#666" }) {
  return <svg width={size} height={size} viewBox="0 0 24 24" fill="none">
    <circle cx="9" cy="8" r="3" stroke={color} strokeWidth="1.7"/>
    <path d="M3 20c0-3.314 2.686-6 6-6s6 2.686 6 6" stroke={color} strokeWidth="1.7" strokeLinecap="round"/>
    <path d="M16 11c1.657 0 3 1.343 3 3M19 20c0-2.21-1.343-4-3-4" stroke={color} strokeWidth="1.7" strokeLinecap="round"/>
  </svg>;
}
function ShieldIcon({ size = 20, color = "#666" }) {
  return <svg width={size} height={size} viewBox="0 0 24 24" fill="none">
    <path d="M12 3L4 6v6c0 4.418 3.582 8 8 9 4.418-1 8-4.582 8-9V6l-8-3z" stroke={color} strokeWidth="1.7" strokeLinejoin="round"/>
  </svg>;
}
function PersonIcon({ size = 20, color = "#666" }) {
  return <svg width={size} height={size} viewBox="0 0 24 24" fill="none">
    <circle cx="12" cy="8" r="4" stroke={color} strokeWidth="1.7"/>
    <path d="M4 20c0-4 3.582-7 8-7s8 3 8 7" stroke={color} strokeWidth="1.7" strokeLinecap="round"/>
  </svg>;
}
function BellIcon({ size = 20, color = "#222" }) {
  return <svg width={size} height={size} viewBox="0 0 24 24" fill="none">
    <path d="M18 8A6 6 0 006 8c0 7-3 9-3 9h18s-3-2-3-9" stroke={color} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"/>
    <path d="M13.73 21a2 2 0 01-3.46 0" stroke={color} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"/>
  </svg>;
}
function ChevronIcon({ size = 14, color = "#555" }) {
  return <svg width={size} height={size} viewBox="0 0 24 24" fill="none">
    <path d="M6 9l6 6 6-6" stroke={color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
  </svg>;
}
function ArrowLeft({ size = 22, color = "#111" }) {
  return <svg width={size} height={size} viewBox="0 0 24 24" fill="none">
    <path d="M19 12H5M5 12l7 7M5 12l7-7" stroke={color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
  </svg>;
}
function UploadIcon({ size = 36, color = "#E53E3E" }) {
  return <svg width={size} height={size} viewBox="0 0 24 24" fill="none">
    <path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8l-6-6z" stroke={color} strokeWidth="1.7" strokeLinejoin="round"/>
    <path d="M14 2v6h6M12 18v-6M9 15l3-3 3 3" stroke={color} strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round"/>
  </svg>;
}
function ClockIcon({ size = 40, color = "#fff" }) {
  return <svg width={size} height={size} viewBox="0 0 24 24" fill="none">
    <circle cx="12" cy="12" r="9" stroke={color} strokeWidth="2"/>
    <path d="M12 7v5l3 3" stroke={color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
  </svg>;
}
function CheckIcon({ size = 36, color = "#fff" }) {
  return <svg width={size} height={size} viewBox="0 0 24 24" fill="none">
    <path d="M5 13l4 4L19 7" stroke={color} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"/>
  </svg>;
}
